...
const Alarm = ({ setAlarms, setAudioURL }) => {
  const [alarms, setLocalAlarms] = useState(JSON.parse(localStorage.getItem('alarms')) || []);
  const [newAlarmTime, setNewAlarmTime] = useState('');
  const [audioName, setAudioName] = useState(localStorage.getItem('audioFileName') || '');
